...
def index():
  ...
  cvs = [(application['user']['cvPath'], application['user']['_id']) for application in applications]
  ...
  for cv_id, cv_content in cvs_content:
    if prediction_model:
      cvs_labels.append((cv_id,cv_content, tokenize(prediction_model.predict([cv_content])[0])))
  ...
  similarities, indexes = calculate_similarity(tfidf_expected_matrix, [generate_tfidf_matrix(cv_label[2]) for cv_label in cvs_labels])
